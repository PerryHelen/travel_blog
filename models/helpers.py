...
# takes -1 or 1 as arguments: whether user reposts or un-reposts a post
def update_reposts(num):
    """
    Takes -1 or 1 as arguments. -1 if user un-reposts a post, and 1 if user reposts a post.
    This function updates the blog statistics database.
    """
    if num == -1 or num == 1:
        stats = Blog_Stats.query.get_or_404(1)
        if num == 1:
            modify_stats = int(stats.reposts_total) + 1
            stats.reposts_total = modify_stats
        else:
            modify_stats = int(stats.reposts_total) - 1
            stats.reposts_total = modify_stats
        db.session.commit()
    else:
        return print("Invalid arguments given to update_reposts function.")
